...
object ChallengeProblems {
  /*
    ...
    Returns - String
  */
  def sameString(s: String): String = s
  ...
  Returns - Int
   */
  def listSize(l: List[Int]): Int = l.size
  ...
  /*
   ...
   Returns - List[String]
*/
  def upper(l: List[String]): List[String] = l.map(s => s.toUpperCase())
  ...
  Returns - List[Int]
   */
  def filterNegatives(l: List[Int]): List[Int] = l.filter(i => i >= 0)
  ...
  Returns - List[String]
 */
  def containsCar(l: List[String]): List[String] = l.filter(s => s.contains("car"))
  ...
  /*
    ...
    Returns - Int
   */
  def sumList(l: List[Int]): Int = l.sum
  ...
  9. Write a function that takes in an integer with a cats age, and return the human age equivalent.
    ...
    Returns - Int
   */
  def catsAge(humanAge: Int): Int = humanAge * 4
  ...
  10. Same question as #9, but this time you are given a Option[Int]
    ...
    -Returns - Option[Int]
 */
  def catsAgeOption(humanAge: Option[Int]): Option[Int] = humanAge.map(age => age * 4)
  ...
  Returns - Int
   */
  def minimum(l: List[Int]): Int = l.min

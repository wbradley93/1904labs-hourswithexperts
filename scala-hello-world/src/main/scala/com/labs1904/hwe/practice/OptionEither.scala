...
object OptionEither {
  ...
  */
  def dogAge(humanAge: Option[Int]): Option[Int] = humanAge.map(age => age * 7)

  /*
    Returns the total cost af any item.
    If that item has a price, then the price + 7% of the price should be returned.
  */
  def totalCost(item: Item): Option[Double] = item.price.map(price => price * 1.07)
  ...
  def averageTemperature(temperatures: List[WeatherStation]): Option[Int] = {
    val suppliedTemps = temperatures.filter(station => station.temperature.isDefined)
    if (suppliedTemps.isEmpty) {
      None
    } else {
      Some(suppliedTemps.foldLeft(0)((a, b) => a + b.temperature.getOrElse(0)) / suppliedTemps.size)
    }
  }
}
